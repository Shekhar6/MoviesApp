...
var MatAutocomplete = class _MatAutocomplete {
  ...
  constructor(_changeDetectorRef, _elementRef, _defaults, platform) {
    ...
    this._activeOptionChanges = import_rxjs4.Subscription.EMPTY;
...
var MatAutocompleteTrigger = class _MatAutocompleteTrigger {
  constructor(_element, _overlay, _viewContainerRef, _zone, _changeDetectorRef, scrollStrategy, _dir, _formField, _document, _viewportRuler, _defaults) {
    ...
    this._viewportSubscription = import_rxjs4.Subscription.EMPTY;
    this._breakpointObserver = inject(BreakpointObserver);
    this._handsetLandscapeSubscription = import_rxjs4.Subscription.EMPTY;
    this._canOpenOnNextFocus = true;
    this._closeKeyEventStream = new import_rxjs4.Subject();
    ...
    this._initialized = new import_rxjs4.Subject();
    this._injector = inject(Injector);
    this._aboveClass = "mat-mdc-autocomplete-panel-above";
    this._overlayAttached = false;
    this.optionSelections = (0, import_rxjs4.defer)(() => {
      ...
      return this._initialized.pipe((0, import_operators3.switchMap)(() => this.optionSelections));
    });
  ...
  get panelClosingActions() {
    return (0, import_rxjs4.merge)(this.optionSelections, this.autocomplete._keyManager.tabOut.pipe((0, import_operators3.filter)(() => this._overlayAttached)), this._closeKeyEventStream, this._getOutsideClickStream(), this._overlayRef ? this._overlayRef.detachments().pipe((0, import_operators3.filter)(() => this._overlayAttached)) : (0, import_rxjs4.of)()).pipe(
      // Normalize the output so we return a consistent type.
      (0, import_operators3.map)((event) => event instanceof MatOptionSelectionChange ? event : null)
    );
  }
  ...
  _getOutsideClickStream() {
    return (0, import_rxjs4.merge)((0, import_rxjs4.fromEvent)(this._document, "click"), (0, import_rxjs4.fromEvent)(this._document, "auxclick"), (0, import_rxjs4.fromEvent)(this._document, "touchend")).pipe((0, import_operators3.filter)((event) => {
  ...
  _subscribeToClosingActions() {
    const initialRender = new import_rxjs4.Observable((subscriber) => {
      afterNextRender(() => {
        subscriber.next();
      }, {
        injector: this._injector
    ...
    const optionChanges = this.autocomplete.options.changes.pipe(
      (0, import_operators3.tap)(() => this._positionStrategy.reapplyLastPosition()),
      // Defer emitting to the stream until the next tick, because changing
      // bindings in here will cause "changed after checked" errors.
      (0, import_operators3.delay)(0)
    );
    return (0, import_rxjs4.merge)(initialRender, optionChanges).pipe(
      // create a new stream of panelClosingActions, replacing any previous streams
      // that were created, and flatten it so our stream only emits closing events...
      (0, import_operators3.switchMap)(() => this._zone.run(() => {
  ...
  _setValueAndClose(event) {
    ...
    if (toSelect) {
      ...
      if (panel._animationDone) {
        panel._animationDone.pipe((0, import_operators3.take)(1)).subscribe(() => this._onChange(null));
      } else {
        this._onChange(null);
      }
    }
